...
test("ast: toContentItems", () => {
  const root = new Ast(
    `<div>abc<img src="image.png" /><audio src="audio.mp3" /><audio><source src="source-audio.mp3" /></audio><video src="video.mp4" /><video><source src="source-video.mp4" /></video>def</div>`,
  );

  eq(root.toContentItems(), [
    {
      type: "text",
      value: "abc",
    },
...
test("ast: toContentItems with nested text", () => {
  const root = new Ast(`<h1>hello <span>nested</span> text</h1>`);
